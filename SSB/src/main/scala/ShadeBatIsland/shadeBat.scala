...
import scala.util.Random
 ...
 object shadeBat {
  def main(args: Array[String]): Unit = {
    ...
    val N =       args(0).toInt // Number of artificial bat (Population length)
    val d =       args(1).toInt //Dimensions ; Number of unknown Decision variables
    val Iterations = args(2).toInt // Maximum Number of Iteration
    val nP =      args(3).toInt // number of partitions
    val migrationFrequency = args(4).toInt //Gap
    val migrationRate =       args(5).toInt //N * 10/100       //Number of Migrants ==== set to 20 %

    //Initialization of Parameters

    val Fmin =    args(6).toDouble // 0 //Minimum Frequency
    val Fmax =    args(7).toDouble // 1//  Maximum Frequency
    val Bandwidth =         args(8).toDouble//0.001
    val InitialPulseRate =  args(9).toDouble //0.1
    val PulseRate = InitialPulseRate
    val LoudnessRate =      args(10).toDouble //0.95
    val alpha =             args(11).toDouble //0.95
    val gyma =              args(12).toDouble //0.95
