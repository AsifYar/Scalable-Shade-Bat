...
class Shade (  dimension : Int  , lower : Int , upper : Int , fun : Array[Double] => Double , mr : Int , N :Int) extends Serializable  {
  def improve(evalutions : Int , pos:Array[Array[Double]]) :  (Array[Double] , Array[Array[Double]] )  = {
    ...
    val population = pos //Array.fill(size, dimension)(random.nextDouble()  *(upper-lower)+lower)
    var memory = population.toList //  Init memory with population
    ...
    while (currentEval < maxEval) {
      ...
      var SF  = Array[Double]()
      val F = Array.fill(size)(0.0)
      val CR = Array.fill(size)(0.0)
      ...
      for ((i, xi) <- population.view.zipWithIndex) {
        ...
        //add mean , multiply standard deviation  // https://www.javamex.com/tutorials/random_numbers/gaussian_distribution_2.shtml
        val Fi = 0.55 //r.nextGaussian() * 0.1 + meanF
        val CRi = 0.95 // r.nextGaussian() * 0.1 + meanCR
        val p = math.random * (0.2 - pmin) + pmin
         //Get two random values
        val ign = Array(xi)
        //val r1: Int = randomIndices(size, 1, ign)(0)

        var r1  =  r.nextInt(size)
        var r2  =  r.nextInt(memory.length)
       ...
       // val r2: Int = randomIndices(memory.length, 1, ign1)(0)
        ...
        v = shade_clip(v, i) // clipping effect
    ...
    /*
    val tmp = population_fitness . zipWithIndex. sortWith(_._1 < _._1).take(mr).unzip
    val FinalbestFitness : Array[Double] = tmp._1   // Fitness
    val FinalbestSolutions : Array[Array[Double]] = tmp._2 map population
    (FinalbestFitness , FinalbestSolutions)
    */
  ...
  def ES(arr: Array[Double], arr1: Array[Double], arr2: Array[Double]): Array[Double] = {
    val newArr =new Array[Double](arr.length)
    //val newArr = Array.fill(arr.length)(new Double(0.0))//
    var i = 0
    while (i < arr.length) {
      newArr(i) = arr(i) + arr1(i) + arr2(i)
      i += 1
 ...
 /* def ES3(arr :Array[Double], arr1 :Array[Double] ) :Array[Double] = {
    ...
    val array =  if ( arr.length < arr1.length) arr.clone() else arr1.clone()
    var i = 0
    while (i < arr.length) {
