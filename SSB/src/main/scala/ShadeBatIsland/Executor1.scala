    ...
    val newRDD = RDD.mapPartitionsWithIndex {
      (index, Iterator) => {
        ...
        if (c_itr != 0) {
          ...
          val recBD1 = recBD.map(_._1) // Mesh topology
        ...
        if (  ApplyBA == true  ) {
          ...
          while (currentEvaluations < Evals) {
            ...
            //.map(_.position) // sort BAT based on fitness
            ...
            var i = 0
            while (i < arr.length) {
              //val loudnessSum: Double = arr.map(_.LoudnessRate).reduce(_ + _)
