...
case class BAT1 ( posVals : Array[Double] , velVals : Array[Double] , f:  Array[Double] =>  Double ,
  ...
  var position      : Array[Double]          =     posVals //Array.fill ( d ) (  random.nextDouble()*(maxx-minn)+minn  )
  var velocity      : Array[Double]          =     velVals //Array.fill ( d ) (  math.random)
  ...
  def move  (  local_Best : Array  [  Double  ]  ,  size : Int ,Fmin : Double , Fmax : Double , Bandwidth : Double,mini:Int , maxi : Int ,
    ...
    if (math.random > PulseRate) {
      newPos = eShadeLsLs(arr , local_Best , local_Best.length ,mini ,maxi ,Bandwidth, sumLoud )  //LocalSearch(local_Best , Bandwidth  ,mini , maxi)
  ...
  def eShadeLsLs( arr : Array[Array[Double]]  ,bestSol : Array[Double]  , iterationss : Int ,mini:Int , maxi : Int , ban : Double ,
    ...
    def loop (best:Array[Double] , fit : Double , iter : Int , indxi : Int):Array[Double] = {
      if( iter == iterationss)
        ...
        val newSol =   best.map( x => keepWithinRange1( (x + ban * sumLoud) * (best(n) -  arr(k0)(n) )
          , mini , maxi, x ))
        val newFit = f (newSol)
        if(newFit <= fit)
